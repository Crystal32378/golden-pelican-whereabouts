...
# Golden Pelican's Whereabouts（鵜鶘踩點地圖）

A companion "WANTED bulletin" site for the golden-pelican project: a world map of every place the pelican has been caught riding, plus a vote on where it strikes next.
...
- "無法定位的地點" column for stations with no earthly coordinates.
- Open-text next-station proposals + a personal ballot box: each visitor drops a pin, names the place (我家巷口 is a feature, not a bug), and casts votes — all stored in their own browser (localStorage). No server, no global counts; your detective notebook is yours. Proposals are shareable as URL-encoded pins.
- Static hosting, no build step, readable on a phone.

## Later
- Auto-syncing station data from the main repo's git log.
- 「畫自己家的河濱騎行」— full draw-your-own-route proposals (trace a path, not just a pin), so someone can nominate their own neighborhood riverside ride.
...
- Multilingual toggle (site copy starts in zh-TW + EN).
